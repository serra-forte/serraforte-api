...
import { FindDiscountCounponUseCase } from './find-by-id-discount-coupons-usecase'

let discountCouponRepositoryInMemory: InMemoryDiscountCounponsRepository
let clinicRepositoryInMemory: InMemoryClinicRepository
let stu: FindDiscountCounponUseCase

describe('Find discount counpon by id (unit)', () => {
  beforeEach(async () => {
    discountCouponRepositoryInMemory = new InMemoryDiscountCounponsRepository()
    clinicRepositoryInMemory = new InMemoryClinicRepository(
      discountCouponRepositoryInMemory,
    )
    stu = new FindDiscountCounponUseCase(discountCouponRepositoryInMemory)
  ...
  test('Should be able to find a discount counpon by id', async () => {
